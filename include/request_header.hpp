...
enum class Method { GET, POST, PUT, DELETE, HEAD, OPTIONS, TRACE, CONNECT, PATCH };
...
public:
  ...
  void set_method(const Method& method) { _method = method; }

  bool contain(const std::string& key) const { return _headers.find(key) != _headers.end(); }
