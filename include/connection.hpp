...
public:
  Connection() = delete;
  Connection(boost::asio::io_context &io_context, T socket,
  ...
  std::shared_ptr<Connection> get_shared_ptr() {
  ...
  bool is_timed_out() const {
    return std::chrono::system_clock::now() - _last_request_time >
...
template <Socket T> void Connection<T>::send(const message::Data &data) {
  boost::asio::async_write(
      _socket,
      boost::asio::buffer(reinterpret_cast<const void *>(data.data()),
                          data.size()),
...
template <Socket T> void Connection<T>::receive(std::uint32_t connection_id) {
  try {
    boost::asio::async_read_until(
        _socket, _buffer, "\r\n\r\n",
        std::bind(&Connection::handle_message, get_shared_ptr(), connection_id,
...
Connection<T>::handle_message(std::uint32_t connection_id,
                              boost::system::error_code ec,
                              std::size_t bytes_transferred) {
  if (!ec) {
    message::Data data{};
    ...
    message::DataBuffer data_buffer{(char *)_buffer.data().data(),
                                    bytes_transferred};
